$(() => {
  ...
  $newReviewForm.on('submit', function(event) {
    ...
    if (!reviewBody) {
      errorMessage = "Please provide some information about this reservation";
    }

    // clear our review fields
    $('#new-review-rating').val("");
    $("#new-review-body").val("");
    if (reviewRating && reservationId && reviewBody && !errorMessage) {
      getIndividualReservation(reservationId)
        .then(data => {
          const dataObj = { ...data, reservation_id: data.id, message: reviewBody, rating: reviewRating };
